...
fn always_error() -> Result<String> {
    Err(Error::from(ThisTestError::NestedStringError {
        msg: "This is the outer error".to_string(),
        source: EnumTraitError::Second("Hello from the EnumTraitError".to_string()),
    })
    .context("ThisError Enum with message and nested error"))
}

fn always_random_error() -> Result<String> {
    match rand::thread_rng().gen_range(0..3) {
        0 => Err(Error::from(EnumSuperError::First {
            source: EnumTraitError::Second("Hello, Keith".to_string()),
        })
        .context("Handmade Enum erros")),
        1 => Err(
            Error::from(ThisTestError::ContainsString("Hello, Keith".to_string()))
                .context("ThisError Enum with String value"),
        ),
        2 => Err(
            Error::from(ThisTestError::NestedError(EnumTraitError::Second(
                "Hello, Keith".to_string(),
            )))
            .context("ThisError Enum with nested error"),
        ),
        3 => Err(Error::from(ThisTestError::NestedStringError {
            msg: "This is the outer error".to_string(),
            source: EnumTraitError::Second("Hello from the EnumTraitError".to_string()),
        })
        .context("ThisError Enum with message and nested error")),
